...
export const refreshToken = async (req, res) => {
  try {
    ...
    return res
      .status(401)
      .json({ message: "Ugyldig eller utløpt innlogging." });
  }
};
...
export const sendResetLink = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ userId: user._id }, process.env.JWT_RESET_SECRET, {
      expiresIn: "1h",
    });
...
export const setNewPassword = async (req, res) => {
  ...
  try {
    ...
    if (!user) return res.status(400).json({message: "Ugyldig eller utløpt lenke."});
